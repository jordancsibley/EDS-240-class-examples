...
us_state_pop <- get_estimates(geography = "state",  # This is where the Sys.getenv("CENSUS_API_KEY") is happening
                              product = "population",
                              state = NULL, 
...
lyme_pop |> 
  ...
  scale_x_discrete(expand = c(0, NA)) +  # Adjust the scale of the x axis (fills the empty space)
...
This kind of plot is difficult to interpret 
...
Order really matters for area charts. Which ever one is on the bottom (on the x axis) is the easiest to interpret and the ones above it take a bit of math to understand. 
